...
class BertForTokenClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.attn_flag = True
        self.attn_flag_2nd_level = False
        self.org_flag = False
        if self.attn_flag:
            self.hidden_dim = 300
            self.context_vector_size = self.hidden_dim * 2
            self.word_GRU = nn.GRU(input_size=768, hidden_size=self.hidden_dim, bidirectional=True, batch_first=True)
            ...
            self.seq_GRU = nn.GRU(input_size=self.hidden_dim*2, hidden_size=self.hidden_dim, bidirectional=True, batch_first=True)
            self.seq_proj = nn.Linear(in_features=2*self.hidden_dim ,out_features=2*self.hidden_dim)
            self.seq_context_vector = nn.Parameter(torch.randn(self.context_vector_size).float())
            init_gru(self.seq_GRU)
            self.classifier = nn.Linear(self.hidden_dim * 2, config.num_labels)
        else:
            if self.org_flag:
                self.classifier = nn.Linear(config.hidden_size , config.num_labels)
            else:
                self.classifier = nn.Linear(config.hidden_size + 850*2, config.num_labels)

        # self.crf = CRF(num_tags=config.num_labels, batch_first=True)

        self.ctc_embed = nn.Embedding(60, 768)
        self.seg_embed = nn.Embedding(60, 768)
        self.init_weights()
    ...
    Examples::
        ...
        sequence_output = outputs[0]
        labels_ctc[labels_ctc<0]=0
        labels_seg[labels_seg<0]=0
        #print(labels_ctc)
        #print(labels_seg)
        #print(labels)
        #import sys
        #sys.exit()
        ctc_embeds = self.ctc_embed(labels_ctc)
        seg_embeds = self.seg_embed(labels_seg)
        #print(sequence_output)
        #print(ctc_embeds)
        #print(seg_embeds)
        #import sys
        #sys.exit()
        #sequence_output = torch.cat([sequence_output, ctc_embeds, seg_embeds], 2)
        if self.attn_flag:
            sequence_output = sequence_output.view(-1, 1, sequence_output.size(2))
            ctc_embeds = ctc_embeds.view(-1, 1, ctc_embeds.size(2))
            seg_embeds = seg_embeds.view(-1, 1, seg_embeds.size(2))
            combo_output = torch.cat([sequence_output, ctc_embeds, seg_embeds], dim=1)
            #print("combo_output: ", combo_output.size()) # 512 * 3 * 768
            combo_output, _ = self.word_GRU(combo_output)
            #print("combo_output: ", combo_output.size()) # 512 * 3 * 600
            combo_output = torch.tanh(self.w_proj(combo_output))
            w_score = self.softmax(combo_output.matmul(self.w_context_vector))
            w_score = w_score.unsqueeze(2)
            sequence_output = combo_output.mul(w_score)
            #print('sequence_output: ', sequence_output.size()) # 512 * 3 * 600
            sequence_output = torch.sum(sequence_output, dim=1, keepdim=True)
            #print('sequence_output: ', sequence_output.size()) # 512 * 1 * 600
            sequence_output = sequence_output.view(-1, 128, sequence_output.size(2)) # bsz * 128 * 600
            #sys.exit()
            if self.attn_flag_2nd_level:
                sequence_output, _ = self.seq_GRU(sequence_output)
                sequence_output = torch.tanh(self.seq_proj(sequence_output))
                w_score = self.softmax(sequence_output.matmul(self.seq_context_vector))
                w_score = w_score.unsqueeze(2)
                #print('sequence_output: ', sequence_output.size())
                #print('w_score: ', w_score.size())
                sequence_output = sequence_output.mul(w_score) # bsz * 128 * 600
        else:
        ...
        # print("log_likelihood.shape: ", log_likelihood.size())
