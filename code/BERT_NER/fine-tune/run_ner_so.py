    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        with open(output_test_results_file, "w") as writer:
            for key in sorted(result.keys()):
                writer.write("{} = {}\n".format(key, str(result[key])))
        # Save predictions
        output_test_predictions_file = os.path.join(args.output_dir, "dev_predictions.txt")
        with open(output_test_predictions_file, "w") as writer:
        ...
        # output_test_results_file = os.path.join(args.output_dir, "train_results.txt")
        # with open(output_test_results_file, "w") as writer:
        #     for key in sorted(result.keys()):
        #         writer.write("{} = {}\n".format(key, str(result[key])))
        # # Save predictions
        # output_test_predictions_file = os.path.join(args.output_dir, "train_predictions.txt")
        # with open(output_test_predictions_file, "w") as writer:
        #     with open(os.path.join(args.data_dir, "test.txt"), "r") as f:
        #         example_id = 0
        #         for line in f:
