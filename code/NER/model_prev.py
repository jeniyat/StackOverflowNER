...
class BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, vocab_size, tag_to_ix, embedding_dim, freq_embed_dim, markdown_embed_dim,seg_pred_embed_dim, hidden_dim, char_lstm_dim=25,
                 char_to_ix=None, pre_word_embeds=None, word_freq_embeds=None, word_markdown_embeds=None, word_seg_pred_embeds=None, char_embedding_dim=25, use_gpu=False,
        ...
        #----------adding segmentation prediction embedding--------
        self.seg_pred_embeds = nn.Embedding(parameters['segmentation_count'], parameters['segmentation_dim'])
        if word_seg_pred_embeds is not None:
            self.use_seg_pred_embed = True
            self.seg_pred_embeds.weight = nn.Parameter(torch.FloatTensor(word_seg_pred_embeds))
        else:
    ...
    def get_lstm_input_dim(self):
        if self.use_elmo:
            if parameters['use_freq_vector'] and parameters['use_markdown_vector'] and parameters['use_segmentation_vector']:
    ...
    def _get_lstm_features_w_elmo(self, sentence_words, sentence, markdown, seg_pred):
    ...
    def neg_log_likelihood(self, sentence_tokens, sentence, sentence_markdowns,sentence_seg_preds, tags, chars2, caps, chars2_length, d):
        ...
        if self.use_elmo:
            if parameters['use_elmo_w_char']:
                ...
                feats = self._get_lstm_features_w_elmo(sentence_tokens, sentence, sentence_markdowns, sentence_seg_preds)
        else:
    ...
    def forward(self,sentence_tokens, sentence, sentence_markdowns, sentence_seg_preds,chars, caps, chars2_length, d):
        if self.use_elmo:
            if parameters['use_elmo_w_char']:
